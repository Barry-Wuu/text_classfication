...
- **原始样本（`is_aug=0`）**：直接从真实投诉场景采集来的原始文本，未做任何改写，是数据集的「真身」，共 6,886 条。
...
**使用时怎么区分？** 训练阶段可利用 `is_aug` 字段做加权、采样或消融实验（比如对比"仅原始"与"原始+增强"的效果差异）；**评估阶段建议优先在原始样本（`is_aug=0`）上报告指标**，因为那才反映真实数据分布下的表现，在增强样本上刷出的高分可能虚高。
