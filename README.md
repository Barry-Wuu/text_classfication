...
在字符级清洗、标签编码与文本张量化完成后，项目对多条建模路线做了**无泄漏**的横向对比（脚本见 [`resources/compare_no_leak.py`](resources/compare_no_leak.py) 与 [`resources/compare_tfidf_clf.py`](resources/compare_tfidf_clf.py)，完整结论见 [`resources/TENSOR.md`](resources/TENSOR.md)）。
...
在**同一份 TF-IDF 特征**（jieba 分词 + 词频，`min_df=2`，25,251 维）上横向对比多个分类器：
