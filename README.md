...
### 5.5 TF-IDF 之上换分类器：11 模型横向对比

固定**同一份 TF-IDF 特征**（jieba 分词 + 词频，`min_df=2`，25,251 维）+ 同一无泄漏划分，横向对比 11 个分类器（fastText 用同一训练/测试集、自行分词训练）。训练集 23,852 条、测试集 1,378 条（全原始）。
...
- **fastText 表现中游**（0.7687）：作为"词袋 + 词向量 + 线性分类"一体的高效基线，36 秒训练拿到 0.7687，优于全部树模型，但比线性 SVM 低约 5 个点——因为这些投诉文本的关键信号集中在**少数强判别词**（品牌名、业务词），词袋线性模型已能充分捕捉，fastText 的 n-gram 子词与浅层嵌入并未带来额外收益。
