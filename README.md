...
固定**同一份 TF-IDF 特征**（jieba 分词 + 词频，`min_df=2`，25,251 维）+ 同一无泄漏划分，横向对比 12 个方法；fastText 用同一训练/测试集自行分词训练，Jev 则直接对测试文本调用大模型 API。训练集 23,852 条、测试集 1,378 条（全原始）。
...
- **大模型 API 并未占优**（Jev 0.7327）：Jev 是 TypeSafe 的 System One 模型，用 Choice 原语把 18 个类名作为候选项做结构化分类（无需解析文本，直接返回判定与置信度，平均置信 0.914）。它在 1378 条测试上拿到 0.7327，**低于线性 SVM 约 9 个点**，仅略高于随机森林。可能原因：全量 18 类细粒度分类对"单次判定"偏难，且该类任务的关键信号是强判别词（品牌/业务词），而 LLM 更擅长开放语义理解而非封闭词表判分；此外它无需训练数据、开箱即用，这是它的优势而非精度优势。
...
- **树模型与大模型 API 均在边界之内**（被支配区）：XGBoost 用 1,426 秒却只有 0.7478；Jev 类 API 方法无需训练但在本任务精度不占优，同样位于边界之内。
