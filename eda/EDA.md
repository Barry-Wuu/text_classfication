...
图 4 把全部 30,000 条样本的长度画成直方图，红色虚线为均值（142），蓝色点划线为中位数（165）。

这张图清楚解释了开头那个"均值小于中位数"的现象：

- **主峰在 150~200**：绝大多数样本长度落在这个区间，正文主体偏长。
...
**左图：长度分布叠加。** 原始样本（蓝）的分布明显更"扁"、覆盖范围更宽；增强样本（橙）则形成一个高而窄的尖峰，集中在 160~190 之间。这说明规则增强很可能是围绕原始样本中的长文本做了规范化改写，导致增强后的长度更趋同。
