...
_SYMBOL_NORM = str.maketrans({
    ...
    "\u2022": "·",     # BULLET • → ·
    "\u25e6": "·",     # WHITE BULLET ◦ → ·
    "\u25cf": "·",     # BLACK CIRCLE ● → ·（实为列表项目符号，与 • ◦ 统一）
    # 上标/下标数字与字母 → 普通形式（10m² → 10m2，H₂O → H2O）
    "\u00b2": "2", "\u00b3": "3", "\u00b9": "1", "\u2070": "0",
    "\u2074": "4", "\u2075": "5", "\u2076": "6", "\u2077": "7",
    "\u2078": "8", "\u2079": "9",
    "\u2080": "0", "\u2081": "1", "\u2082": "2", "\u2083": "3",
...
def scan(rows):
    ...
    stat = {
        ...
        "含上/下标数字": sum(1 for r in rows if re.search(r"[\u00b2\u00b3\u00b9\u2070-\u2079\u2080-\u2089\u207a\u207b\u207f]", r["text"])),
        "含其它语言字符": sum(1 for r in rows if re.search(r"[\u0370-\u03ff\u0400-\u04ff\u0530-\u058f\u0590-\u05ff\u0600-\u06ff\u0900-\u0dff\u0e00-\u0fff\u1000-\u109f\u10a0-\u10ff\u1100-\u11ff\u3040-\u30ff\u3100-\u312f\u3130-\u318f\uff66-\uff9f]", r["text"])),
