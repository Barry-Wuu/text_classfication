...
def main():
    ...
    print("学生参数量 %.2fM" % (sum(p.numel() for p in student.parameters()) / 1e6),
          flush=True)
    optim = torch.optim.AdamW(student.parameters(), lr=1.5e-3, weight_decay=0.01)
    ...
    record("蒸馏LSTM学生 fp32", student, DEV_GPU, preds, gts,
           note="T=2 a=0.7 全量%d条x%d轮(256维学生), GPU延迟" % (len(train_texts), EPOCHS))
