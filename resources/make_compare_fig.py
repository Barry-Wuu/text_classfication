...
# 颜色：线性模型蓝、树模型橙、贝叶斯绿、kNN 灰、fastText 紫、大模型红
def color(n):
...
# 右：耗时（对数轴）—— 本地模型为训练耗时，Jev 为 1378 条推理总耗时
ax = axes[1]
ax.set_facecolor(BG)
...
ax.set_title("耗时（本地模型=训练；Jev=1378条推理总耗时）", fontsize=12, fontweight="bold")
...
fig.suptitle("消费者投诉文本分类 · 12 个方法横向对比", fontsize=15, fontweight="bold", y=0.99)
fig.text(0.5, 0.005,
         "蓝=线性模型　橙=树模型　紫=fastText　红=大模型 API(Jev)　绿=朴素贝叶斯　灰=kNN（同一无泄漏划分）",
