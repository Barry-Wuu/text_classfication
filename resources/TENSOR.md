...
补充（打榜视角）：把上述方法画到"耗时（横轴，对数）—macro-F1（纵轴）"平面上，帕累托最优边界只含 3 个成员——**ComplementNB**、**SGD**（hinge 损失）与 **LinearSVC**，它们同时占据"快"与"准"的前沿；树模型与大模型 API 全部落在边界之内（被支配区）。图见 `resources/figures/10_pareto.png`。即在本任务上快与准并不冲突，最好的三个方法恰好也是最快的三个。
