...
### 3.1 先踩的坑：增强样本泄漏导致分数虚高

第一版对比用的是**普通随机 8:2 划分**，得到 TF-IDF macro-F1 = 0.9927，高得可疑。追查后发现问题出在**数据增强的泄漏**上：

本数据集 77% 是规则增强样本，增强样本的 `id` 形如 `17401489078#4`，`#` 前即**源原始样本 id**。随机划分时，**同一原始投诉及其改写变体被切到了 train/test 两侧** —— 实测随机划分下，96.0% 的测试样本，其同源族都出现在训练集（加粗：**96.0%**）。TF-IDF 于是不是在"学分类"，而是在"认同一句话的变体"，分数被灌到 0.99。

| 划分方式 | TF-IDF macro-F1 |
|---|---:|
| 普通随机划分（有同源泄漏） | 0.9918 |
| 按源 id 分组划分（无泄漏） | **0.8300** |

**16 个点的差距全部来自泄漏**。核查脚本：`resources/leak_check.py`。

### 3.2 无泄漏重比（最终结论）

评估协议：训练含全部增强样本、测试只用原始样本；按源 id 分组，测试源的全部增强变体排除出训练。
实测环境：Kaggle 云端 GPU（torch 2.10+cu128），数据从 GitHub `Barry` 分支拉取 `train_augmented.csv`。
...
1. **真实泛化水平就是约 0.82~0.84**，此前 0.99 是泄漏虚高，对外一律以本节数字为准。
2. 部署推荐 B——**TF-IDF + 线性 SVM**：比 BERT 微调只低 **1.5 个百分点**（0.8221 vs 0.8371），
   但训练只需 **13 秒 / CPU**（BERT 要 885 秒 / GPU，差 67 倍）、推理毫秒级、模型几 MB、零框架依赖。
   对"投诉自动分流"场景，1.5 个点换不来一个 GPU 依赖，**TF-IDF + SVM 是性价比最优解**。
3. **BERT 微调略优但代价高昂**：仅在机器需理解未见的语义改写、且能承担 GPU 成本时才值得上。
4. 词频随机嵌入最差（**0.4742**）：随机初始化嵌入无预训练知识、均值池化又抹平词序，泄漏一撤即原形毕露。
5. **暴露出的数据问题**：本数据集因增强变体共享领域词，**任何随机划分的评估都会被系统性高估**；
   后续所有报告指标都必须采用**按源 id 分组**的划分。
