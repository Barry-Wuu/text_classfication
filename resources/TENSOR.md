...
实测环境：Kaggle 云端 GPU（Tesla P100，torch 2.10+cu128），数据从 GitHub `Barry` 分支拉取 `labeled.csv`。
同一 8:2 分层划分（seed=42，训练 24,000 / 测试 6,000，18 类）、同一 LinearSVC（C=1.0）分类头，
只替换"文本如何变张量"：

| 策略 | 特征维度 | macro-F1 | 准确率 | 训练耗时 | 说明 |
|---|---:|---:|---:|---:|---|
| A. BERT [CLS] 句向量 | 768 | 0.9009 | 0.8963 | 65.3s | 不微调，纯特征提取（向量提取 321s） |
| B. 词频 EmbeddingBag | 256 | 0.7083 | 0.7055 | 13.4s | 随机嵌入 + 均值池化（无监督） |
| **C. TF-IDF 稀疏** | 28807 | **0.9927** | **0.9922** | **1.5s** | 传统基线，**最优** |

脚本：`resources/compare_tensors.py`（另有一套等价的 Kaggle 云端版本）。

### 结论与推荐

1. **推荐方案 C**（TF-IDF + 线性分类器）：macro-F1 0.9927、训练仅 1.5 秒，三项全胜。
2. **BERT [CLS] 不做微调时仅 0.9009**：预训练句向量承载的是通用语义，反而丢掉了本任务关键的
   领域专有词信息；且向量提取一次要 321 秒（GPU），性价比低。若要用 BERT，正确做法是**微调**，
   而不是拿 [CLS] 当固定特征。
3. **词频 EmbeddingBag 最差**（0.7083）：随机初始化嵌入无任何预训练知识，均值池化又抹平了词序，
   本质是"无权重词袋"，说明**无监督随机嵌入在本任务上几乎没有价值**。
4. **对 0.9927 的合理性核查**：已确认 train/test 无重复文本（交集 0），非数据泄漏；
   高分源于本数据集 18 个消费领域**词汇专有性极强**（品牌名、领域术语几乎构成类别指纹），
   这与 EDA 结论"品牌名是最强类别信号"一致。
