# -*- coding: utf-8 -*-
"""Kaggle 云端：BERT 续训 —— 从上游内核「最后一轮(epoch30)」的权重继续训练，直到测试准确率明显下降（过拟合）。
...
上游：wubarry/textcls-bert-continue 的 output（kernel_sources 挂载），其中有
      bert_epoch30.pt（上一阶段最后一轮权重）与 result.json（epoch1~30 曲线）。
"""
...
SEED = 42
MAX_LEN = 128
START_EPOCH = 31          # 第一轮续训的序号（前 10 轮已在上游跑完）
MAX_EPOCH = 60            # 硬上限
...
UPSTREAM_SLUG = "textcls-bert-continue"
