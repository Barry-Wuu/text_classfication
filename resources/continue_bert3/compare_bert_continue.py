# -*- coding: utf-8 -*-
"""Kaggle 云端：BERT 续训 —— 从上游内核「最后一轮(epoch60)」的权重继续训练，直到测试准确率明显下降（过拟合）。
...
硬上限 MAX_EPOCH=200 兜底。

上游：wubarry/textcls-bert-continue2 的 output（kernel_sources 挂载），其中有
      bert_epoch60.pt（上一阶段最后一轮权重）与 result.json（epoch1~60 曲线）。
"""
...
SEED = 42
MAX_LEN = 128
START_EPOCH = 61          # 第一轮续训的序号（epoch1~60 已在上游各阶段跑完）
MAX_EPOCH = 200           # 硬上限
...
def main():
    ...
    for epoch in range(START_EPOCH, MAX_EPOCH + 1):
        ...
        # 体积控制：Kaggle output 上限 20GB、单个权重约 390MB。
        # 仍每轮保存一次权重，但只滚动保留最近 3 轮（最佳权重另存 bert_best.pt）。
        _old = os.path.join("/kaggle/working", f"bert_epoch{epoch - 3}.pt")
        ...
        if test_acc > best_acc:
            ...
            flag = "  <- 新最佳"
            torch.save({"state_dict": model.state_dict(), "num_labels": len(cats),
                        "epoch": epoch}, "/kaggle/working/bert_best.pt")
